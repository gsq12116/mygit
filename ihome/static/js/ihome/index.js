...
$(document).ready(function(){
    $.get('/index/check_user/', function(data){
        if(data.code == 200){
            $(".register-login").hide();
            $(".user-info").show();
            $(".user-info span").append("<a href='/user/my/'>"+ data.user.phone +"</a>");
        }
    });
    $.get('/user/area_info/', function(data){
        for (var i=0; i< data.area_infos.length; i++){
            $('.area-list').append("<a href='#' area-id='"+ data.area_infos[i].id +"'>"+ data.area_infos[i].name +"</a>");
        }
