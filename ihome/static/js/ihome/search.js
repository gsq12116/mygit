...
function updateHouseData(action) {
    ...
    $.post('/index/search_info/', params, function(data){
        var houses_html = template('house_item_list', {houses: data})
...
$(document).ready(function(){
    ...
    $.get('/user/area_info/', function(data){
        for (var i=0; i< data.area_infos.length; i++){
            if (data.area_infos[i].id == areaId){
                $('.filter-area').append("<li area-id='"+ data.area_infos[i].id +"' class='active' >"+ data.area_infos[i].name +"</li>");
            }
            else{
                $('.filter-area').append("<li area-id='"+ data.area_infos[i].id +"'>"+ data.area_infos[i].name +"</li>");
            }
        }
